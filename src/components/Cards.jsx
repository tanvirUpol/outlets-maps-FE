...
const Cards = ({filteredData}) => {
    ...
    const sameStore = filteredData.filter((obj) => obj['ff_this'] > 0 && obj['ff_last'] > 0);
    ...
    //@desc All store Sales Calculations
        ...
        const AllSalesDiffPercentage = calculatePercentage(sumSalesThisAllStore, sumSalesLastAllStore)
    ...
    //@desc same store Sales Calculations
        ...
        const SameSalesDiff = (sumSalesThisSameStore - sumSalesLastSameStore).toFixed(2); 

        const SameSalesDiffPercentage = calculatePercentage(sumSalesThisSameStore, sumSalesLastSameStore)
    ...
    //@desc all store FF Calculations
        ...
        const AllFFDiffPercentage = calculatePercentage(sumFFThisAllStore, sumFFLastAllStore)
    ...
    //@desc same store FF Calculations
      ...
      const SameFFDiffPercentage = calculatePercentage(sumFFThisSameStore, sumFFLastSameStore)

      //@desc all store BS Calculations

        const avgBSThisAllStore = (sumSalesThisAllStore / sumFFThisAllStore).toFixed(2); 
        const avgBSLastAllStore = (sumSalesLastAllStore / sumFFLastAllStore).toFixed(2); 

        const AllBSDiff = (avgBSThisAllStore - avgBSLastAllStore).toFixed(2); 

        const AllBSDiffPercentage = calculatePercentage(avgBSThisAllStore, avgBSLastAllStore)
      
      //@desc same store BS Calculations
  
        const avgBSThisSameStore = (sumSalesThisSameStore / sumFFThisSameStore).toFixed(2); 
        const avgBSLastSameStore = (sumSalesLastSameStore / sumFFLastSameStore).toFixed(2); 

        const SameBSDiff = (avgBSThisSameStore - avgBSLastSameStore).toFixed(2); 

        const SameBSDiffPercentage = calculatePercentage(avgBSThisSameStore, avgBSLastSameStore)
      
      //@desc all store GPV Calculations
        const sumGPVThisAllStore = allStore.reduce(
          (sum, obj) => sum + obj['gpv_this'],
          0
        ).toFixed(2);
        const sumGPVLastAllStore = allStore.reduce(
          (sum, obj) => sum + obj['gpv_last'],
          0
        ).toFixed(2);

        const AllGPVDiff = (sumGPVThisAllStore - sumGPVLastAllStore).toFixed(2); 

        const AllGPVDiffPercentage = calculatePercentage(sumGPVThisAllStore, sumGPVLastAllStore)

        //@desc same store GPV Calculations

          const sumGPVThisSameStore = sameStore.reduce(
            (sum, obj) => sum + obj['gpv_this'],
            0
          ).toFixed(2);

          const sumGPVLastSameStore = sameStore.reduce(
            (sum, obj) => sum + obj['gpv_last'],
            0
          ).toFixed(2);
          ...
          const SameGPVDiffPercentage = calculatePercentage(sumGPVThisSameStore, sumGPVLastSameStore)
    ...
    const totalProfitableStores = filteredData.filter(item => item.profitable === 'Profitable').length;
    const totalNonProfitableStores = filteredData.filter(item => item.profitable !== 'Profitable').length;
    ...
    return (
     <>
              ...
              <Link href="#" className="block mx-1 w-full p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100">
